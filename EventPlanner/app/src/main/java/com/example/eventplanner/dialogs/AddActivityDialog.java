...
public class AddActivityDialog {
    ...
    private static LocalDateTime eventStartDateTime;
    ...
    public interface AddActivityListener {
        void onActivityCreated(String activityName, String description, String location, LocalDateTime startDateTime, LocalDateTime endDateTime);
    }

    public static void show(LocalDateTime dateTime, EventActivity lastActivity, Context context, AddActivityDialog.AddActivityListener listener) {
        LayoutInflater inflater = LayoutInflater.from(context);
        dialogView = inflater.inflate(R.layout.dialog_add_activity, null);
    ...
    private static void initializeViews(Context context) {
        ...
        if (activity != null) {
            startDate.setText(activity.getEndDate().toLocalDate().toString());
            startTime.setText(activity.getEndDate().toLocalTime().toString());
        } else {
            startDate.setText(eventStartDateTime.toLocalDate().toString());
            startTime.setText(eventStartDateTime.toLocalTime().toString());
        }
    ...
    private static void createDialog(Context context, AddActivityDialog.AddActivityListener listener) {
        MaterialAlertDialogBuilder builder = new MaterialAlertDialogBuilder(context);
        builder.setView(dialogView)
                .setTitle("Create Agenda Activity")
        ...
        dialog.setOnShowListener(dialogInterface -> {
            dialog.getButton(android.app.AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
                ...
                if (!isValid) {
                    ...
                    listener.onActivityCreated(activityNameTrimmed, descriptionTrimmed, locationTrimmed, LocalDateTime.parse(startDateTrimmed + "T" + startTimeTrimmed + ":00"), LocalDateTime.parse(endDateTrimmed
                    + "T" + endTimeTrimmed + ":00"));
